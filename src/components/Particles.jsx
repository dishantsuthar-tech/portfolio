  ...
  void main() {
    ...
    if (uAlphaParticles < 0.5) {
      ...
      gl_FragColor = vec4(
        vColor +
        0.2 *
        sin(uv.yxx + uTime + vRandom.y * 6.28),
        1.0
      );
...
const Particles = ({
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      // Mouse position relative to the Particles container
      const x = ((e.clientX - rect.left) / rect.width) * 2 - 1;

      const y = -(((e.clientY - rect.top) / rect.height) * 2 - 1);

      // Keep values inside a reasonable range
      mouseRef.current = {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const col = hexToRgb(selectedColor);
